...
test("fixed crop detail evidence exposes node attachments and rosette separation", async () => {
  ...
  frame(gpu, (current: Frame) => current.pass(
    { target: nodeOutput, clear: CLEAR, clearDepth: 1 },
    (pass: FramePass) => pass.draw(crop),
  ));
  const nodePixels = await nodeOutput.read();
  ...
  frame(gpu, (current: Frame) => current.pass(
    { target: rosetteOutput, clear: CLEAR, clearDepth: 1 },
    (pass: FramePass) => pass.draw(crop),
  ));
  const rosettePixels = await rosetteOutput.read();
