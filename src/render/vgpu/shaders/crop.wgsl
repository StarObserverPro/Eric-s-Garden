...
fn vs_main(input: VertexIn) -> VertexOut {
  ...
  // Shape, taper and clearance are authored with their normals in crop-geometry.
  // Never stretch just a blade here: its petiole and attachment would stay behind.
  ...
  // One continuous field per plant: coincident points remain coincident regardless of material/flex.
  let stiffness = select(0.62, 0.40, abs(crop_kind - 2.0) < 0.25);
  let wind_amplitude = gust * uniforms.scene.y * stiffness * 0.048;
  let wind_amount = wind_amplitude * height_load;
  let height_t = clamp((local.y - 0.04) / 1.16, 0.0, 1.0);
  let wind_derivative = wind_amplitude * 6.0 * height_t * (1.0 - height_t) / 1.16;
  local.x += wind_amount * -0.84;
  local.z += wind_amount * 0.54;
  // Inverse transpose of x+=f(y)*dx, z+=f(y)*dz (not an arbitrary normal offset).
  normal = normalize(vec3f(normal.x, normal.y - wind_derivative * (-0.84 * normal.x + 0.54 * normal.z), normal.z));
...
fn foliage_color(crop: f32, variation: f32) -> vec3f {
  ...
  if (crop < 0.5) { color = vec3f(0.17, 0.48, 0.115); }
  else if (crop < 1.5) { color = vec3f(0.235, 0.53, 0.14); }
  else if (crop < 2.5) { color = vec3f(0.24, 0.52, 0.12); }
  else if (crop < 3.5) { color = vec3f(0.19, 0.49, 0.10); }
  else if (crop < 4.5) { color = vec3f(0.31, 0.57, 0.16); }
  else { color = vec3f(0.19, 0.49, 0.11); }
...
fn harvest_color(crop: f32, stage: f32, variation: f32) -> vec3f {
  ...
  if (crop < 0.5) { color = vec3f(0.95, 0.39, 0.055); }
  else if (crop < 1.5) {
...
fn fs_main(input: VertexOut, @builtin(front_facing) front_facing: bool) -> @location(0) vec4f {
  if (input.visible_flag < 0.5) { discard; }
  ...
  if (is_foliage && (crop > 0.5) && (crop < 1.5)) { direct_floor = 0.094; }
  if (is_stem) {
  ...
  if (is_foliage || is_husk) {
    let leaf_wave = 0.5 + 0.5 * sin(input.world.x * 18.0 + input.world.z * 15.0 + input.world.y * 13.0 + variation * 6.2831853);
  ...
  // Only thin surfaces are intentionally two-sided. Solid fruit/stem normals remain outward.
  let thin_surface = is_foliage || is_husk;
  let sided_normal = select(interpolated_normal, -interpolated_normal, thin_surface && !front_facing);
  var face_normal = normalize(cross(dpdx(input.world), dpdy(input.world)));
  // Screen-space derivative handedness is not a world-space outward-normal convention.
  if (dot(face_normal, sided_normal) < 0.0) { face_normal = -face_normal; }
  if (!front_facing && thin_surface) {
    albedo *= vec3f(0.94, 1.02, 0.91);
    roughness = min(0.94, roughness + 0.07);
    backlight_strength *= 1.12;
  }
  // Fruit is curved, not a 72% flat-shaded triangle fan; keep a restrained faceted accent on leaves.
  var normal = normalize(mix(sided_normal, face_normal, 0.10));
  if (thin_surface) { normal = normalize(mix(sided_normal, face_normal, 0.22)); }
  ...
  if (is_foliage || is_husk) { base_occlusion = 0.82 + 0.18 * smoothstep(-0.02, 0.40, input.world.y); }
  ...
  if (is_harvest) {
    ...
    if ((crop > 2.5) && (crop < 3.5)) {
      ...
      let fruit_center = vec2f(0.28, 0.33) * whole_scale;
  ...
  if (is_harvest) { color += uniforms.lightColor.rgb * rain * 0.025; }
